<script setup>
	...
	const mkaccount = ref({
		...
		exec: () => {
			...
			const xhr = postGas(
				...
				{
					loadend: () => {
						mkaccount.value.trying = false;
					},
					abort: () => {
						mkaccount.value.error = "問題が発生しました";
					},
					error: () => {
						mkaccount.value.error = "問題が発生しました";
					},
					load: () => {
						try {
							...
							mkaccount.value.error =
								"アカウントを作成しました!利用可能になるまで時間がかかります";
	...
	const someTrying = computed(
		() =>
			mkaccount.value.trying ||
...
<template>
	<div
		class="flex justify-center items-center w-full relative p-7.5 bg-white"
	>
		<form class="mt-10 relative text-center" @submit.prevent="dologin.exec">
			<div>
				<h1 class="text-2xl text-center font-bold mt-2.5 mb-3.75">
					ログイン
				</h1>
				<div class="flex flex-col text-2xl relative text-start w-full">
					<div class="mt-2 w-full inline-flex flex-col gap-4">
						...
						>
							<input
								class="px-3.5 py-2 w-full shadow-none caret-blue-200"
								v-model="dologin.userId"
								v-bind:readonly="someTrying"
								placeholder="ユーザ名"
							/>
						</div>
						<div
					...
					<p class="error">
						<LoadingIndicator v-bind:show="dologin.trying" />
						{{ dologin.error }}
					</p>
					<div class="items-center flex justify-center mt-5">
						<div class="bg-neutral-300 h-0.25 flex-grow"></div>
						<div
							class="text-neutral-300 flex-grow-0 px-2 text-[15px] font-bold"
						>
							または
						</div>
						<div class="bg-neutral-300 h-0.25 flex-grow"></div>
			...
			<div class="items-center flex flex-col pb-2.5 pt-10">
				...
				<div class="font-bold text-neutral-500">
					アカウントをお持ちではないですか?
					<span
						class="text-blue-400 cursor-pointer ml-1.25 text-[15px]"
						>登録</span
					>
				</div>
			</div>
